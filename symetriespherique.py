...
class SymetrieSpherique2D(Scene):
    def construct(self):
        ...
        # ==========================================================
        # ZONE DE LÉGENDE (bas de l'écran)
        # ==========================================================
        bg_rect = Rectangle(
        ...
        def make_legend(tex_str):
            leg = Tex(tex_str, font_size=36)
            leg.move_to(bg_rect.get_center())
            return leg
        ...
        # 12 vecteurs radiaux régulièrement espacés pour bien couvrir les 360°
        ...
        # Vecteur rouge = le vecteur E qu'on va "mettre en hypothèse" et torturer par symétrie
        ...
        # À droite du vecteur pour ne pas se superposer au disque de charge
        ...
        # ==========================================================
        # On fond le vecteur rouge vers un vecteur blanc de même style/longueur que les
        # autres — visuellement, E(P) rejoint le groupe et devient "comme les autres".
        target_white_vec = make_arrow(
            P_start, P_start + np.array([0, 0.6, 0]),  # même style que radial_vectors
            color=WHITE, stroke_width=3, tip_size=0.12,
        )
        self.play(Transform(red_vector, target_white_vec), FadeOut(P_dot), FadeOut(P_label), run_time=1.2)
        self.wait(0.6)

        # La sphère apparaît maintenant — l'argument sur la norme constante vient juste après
        leg_4 = make_legend(
        ...
        # Décalé perpendiculairement à la ligne pour ne pas la masquer
        ...
        # Argument clé : deux points à la même distance R sont reliés par une rotation
        # qui laisse la charge invariante → ||E(P1)|| = ||E(P2)||. On va le montrer
        # visuellement en faisant pivoter P1 sur P2.
        leg_4b = make_legend(
        ...
        # Deux points choisis sur le cercle de Gauss — espacés pour que la rotation soit bien visible
        angle1 = 70 * DEGREES
        angle2 = -20 * DEGREES
        ...
        # Mêmes longueurs dès le départ : c'est précisément ce qu'on cherche à prouver
        ...
        # L'arc montre visuellement le chemin parcouru par P1 pour rejoindre P2
        rot_arc = Arc(
            radius=R_gauss,
            start_angle=angle1,
        ...
        # Le groupe (P1 + son vecteur E) pivote jusqu'à se superposer exactement sur P2
        ...
        # Les deux vecteurs sont maintenant superposés : conclusion visuelle claire
        leg_4d = make_legend(
        ...
        # On nettoie tout SAUF le rectangle de légende (qu'on garde)
        everything = VGroup(
            title_part1, charge, plus_sign, red_vector,
            radial_vectors, gauss, r_line, r_label, gauss_vectors, leg_4_final
        )
        ...
        # ==========================================================
        # INTRO "3D" : vue en perspective du plan infini
        # On simule la 3D avec un parallélogramme + vecteurs inclinés.
        # Pas de vraie 3D (trop lent), juste de la perspective 2D.
        # ==========================================================
        # Parallélogramme représentant le plan vu en perspective
        # Vecteur "profondeur" (vers le fond droit)
        ...
        leg_p2_6 = make_legend(
            "Regardons les contributions $d\\vec{E}$ de chaque élément $d\\ell$ du plan."
        )
        ...
        for xdl in dl_positions_x:
            ...
            dl_dots.add(d)
        dl_label = Tex("$d\\ell$", font_size=26).set_color(BLUE_A)
        ...
        # On a un nombre FINI d'éléments dl, donc la somme n'est que une approximation.
